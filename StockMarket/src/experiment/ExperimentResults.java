...
public class ExperimentResults {
    ...
    public void toCSV(int type,String file_path){
        ...
        for(int i = 0; i<results.size();i++){
            double value;
            switch (type){
                case ACCURACY: value = results.get(i).getCorrectsMean(); break;
                case TP: value = results.get(i).getTruePositivesMean(); break;
                case TN: value = results.get(i).getTrueNegativesMean(); break;
                case FP: value = results.get(i).getFalsePositivesMean(); break;
                case FN: value = results.get(i).getFalseNegativesMean(); break;
                default: value = results.get(i).getCorrectsMean(); break;
            }
            
            if(results.get(i).getAlgorithm().equals(dataset)){
                line = line+","+value;
            }else{
                lines.add(line);
                dataset = results.get(i).getAlgorithm();
                line = dataset+","+value;
