    ...
    private static Instances generateEmptyNominalDataSet(String name, List<Pregao> historico, int window) {
        if (window > 0) {
            ...
            for (int i = window; i >= 0; i--) {
                if (i > 0) {
                    abertura[window - i] = new Attribute("AberturaDM" + Math.abs(i * (-1)), labels);
                    baixa[window - i] = new Attribute("BaixaDM" + Math.abs(i * (-1)), labels);
                    alta[window - i] = new Attribute("AltaDM" + Math.abs(i * (-1)), labels);
                    volume[window - i] = new Attribute("VolumeDM" + Math.abs(i * (-1)), labels);
                    fechamento[window - i] = new Attribute("FechamentoDM" + Math.abs(i * (-1)), labels);
